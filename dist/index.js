// Playhub Metadata 1.8.1: rebuilt from src with TypeScript 5.8.3.
const index = (() => {
...
const injectMetadataMenuItem = (menu, appId) => {
    if (!react_1.default.isValidElement(menu) || !appId || !(0, steam_1.isNonSteamApp)((0, steam_1.getOverview)(appId)))
        return menu;
    const children = menu.props?.children;
    const items = (Array.isArray(children) ? children : [children])
        .filter((node) => node?.key !== ENTRY_KEY);
    const propertiesIndex = items.findIndex((node) => !!(0, ui_1.findInReactTree)(node, (item) => (0, compat_1.functionSource)(item?.props?.onSelected ?? item?.onSelected).includes("AppProperties")));
    items.splice(propertiesIndex >= 0 ? propertiesIndex : items.length, 0, (0, jsx_runtime_1.jsx)(ui_1.MenuItem, { onSelected: () => ui_1.Navigation.Navigate(`/playhub-metadata/${appId}`), children: (0, i18n_1.t)("editMetadata") }, ENTRY_KEY));
    return react_1.default.cloneElement(menu, { children: items });
};
